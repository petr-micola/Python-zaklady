...
persons.extend([('Martin', 60, 'muž',), ('Marie', 36, 'žena'), ('Jarek', 35, 'muž'), ('Adéla', 22, 'žena')])

women = [item for item in persons if item[2] == 'žena']
# nebo: women = list(filter(lambda item: item[2] == 'žena', persons))
for woman in women:
    print(woman[0])
    for letter in woman[0]:
        print('-', end='')
    print('')

ipeople = [item for item in persons if 'i' in item[0]]
ipeople = list(filter(lambda item: 'i' in item[0], persons))
ipeople.sort(key=lambda item: item[1], reverse=True)
sloupce = 'index;jmeno;vek;pohlavi'
print(sloupce)
print(*[f'{i};{item[0]};{item[1]};{item[2]}\n' for i, item in enumerate(ipeople)])
=======
'''
V jazyce Python můžeme používat několik tzv. složených datových typů, které slouží k uložení většího počtu hodnot.
Nejuniverzálnější z nich je seznam - list.
